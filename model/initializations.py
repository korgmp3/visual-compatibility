...
def weight_variable_truncated_normal(input_dim, output_dim, name=""):
    ...
    initial = tf.compat.v1.truncated_normal([input_dim, output_dim], stddev=0.5)
...
def weight_variable_random_uniform(input_dim, output_dim=None, name=""):
    ...
    if output_dim is not None:
        ...
        initial = tf.compat.v1.random_uniform([input_dim, output_dim], minval=-init_range, maxval=init_range, dtype=tf.float32)
    else:
        init_range = np.sqrt(6.0 / input_dim)
        initial = tf.compat.v1.random_uniform([input_dim], minval=-init_range, maxval=init_range, dtype=tf.float32)
    return tf.Variable(initial, name=name)
...
def bias_variable_truncated_normal(shape, name=""):
    ...
    initial = tf.compat.v1.truncated_normal(shape, stddev=0.5)
    return tf.Variable(initial, name=name)
...
def bias_variable_const(shape, val, name=""):
    """Create a bias variable initialized as zero."""
    value = tf.compat.v1.to_float(val)
